...
export class BaseCustomWebComponentNoAttachedTemplate extends HTMLElement {
    ...
    if (node instanceof Element) {
      for (let a of node.attributes) {
        if (a.name.startsWith('@') && !a.value.startsWith('[[')) {
          ...
          try {
            if (a.name == "@touch:contextmenu") addTouchFriendlyContextMenu(node, this[a.value].bind(this));else node.addEventListener(a.name.substr(1), this[a.value].bind(this));
          } catch (error) {
    ...
    if (node instanceof Element) {
      ...
      for (let a of attributes) {
        if (a.name.startsWith('css:') && a.value.startsWith('[[') && a.value.endsWith(']]')) {
          ...
          if (a.name == "@touch:contextmenu") addTouchFriendlyContextMenu(node, e => this._bindingRunEval(value, repeatBindingItems, e));else node.addEventListener(camelCased, e => this._bindingRunEval(value, repeatBindingItems, e));
  ...
  _bindingsRefresh() {
    if (this._bindings) this._bindings.forEach(x => x(false));
  }
