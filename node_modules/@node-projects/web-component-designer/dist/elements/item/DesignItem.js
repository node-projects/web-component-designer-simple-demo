  ...
  *children() {
  ...
  insertChild(designItem, index) {
    ...
    if (index == null || this._childArray.length == 0 || index >= this._childArray.length) {
      ...
      if (el) {
        if (el.nodeType === NodeType.Element) el.element.insertAdjacentElement('beforebegin', designItem.element);else if (el.nodeType === NodeType.TextNode) el.element.insertAdjacentText('beforebegin', designItem.node.textContent);
      } else {
  ...
  static createDesignItemFromInstance(node, serviceContainer, instanceServiceContainer) {
    ...
    if (designItem.element.children.length === 0) designItem.content = designItem.element.textContent;else {
      for (const c of designItem.element.children) designItem._childArray.push(DesignItem.createDesignItemFromInstance(c, serviceContainer, instanceServiceContainer));
    }
    return designItem;
  }
