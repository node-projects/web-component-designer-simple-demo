...
export class ItemsBelowContextMenu {
  ...
  provideContextMenuItems(event, designerView, designItem) {
    ...
    if (lstItems.length > 0) {
      ...
      return [{
        ...
        title: 'select: ' + x.localName + (x.id ? ' (' + x.id + ')' : ''),
        action: () => this._select(designerView, x)
      }))];
    }
  ...
  _searchForItemsBelow(event, designerView) {
    const lstEl = []; //search for containers below mouse cursor.
    ...
    try {
      ...
      if (el !== designerView.rootDesignItem.element) {
        ...
        while (el != null) {
          if (el === designerView.rootDesignItem.element) break;
          if (el !== designerView.overlayLayer && el.parentElement !== designerView.overlayLayer && el.getRootNode() === designerView.shadowRoot) lstEl.push(el);
          backupPEventsMap.set(el, el.style.pointerEvents);
          el.style.pointerEvents = 'none';
          const oldEl = el;
          el = designerView.elementFromPoint(event.x, event.y);
