...
export class DesignerView extends BaseCustomWebComponentLazyAppend {
  ...
  _pointerEventHandler(event) {
    ...
    if (this._lastHoverDesignItem != currentDesignItem) {
      if (this._lastHoverDesignItem) this._extensionManger.removeExtension(this._lastHoverDesignItem, ExtensionType.MouseOver);
      if (currentDesignItem && currentDesignItem != this.rootDesignItem && currentElement.parentNode !== this.overlayLayer) this._extensionManger.applyExtension(currentDesignItem, ExtensionType.MouseOver);
  ...
  _pointerActionTypeDragOrSelect(event, currentDesignItem, currentPoint) {
    if (event.altKey) {
      ...
      let currentElement = this.shadowRoot.elementFromPoint(event.x, event.y);
      if (currentElement.parentNode !== this.overlayLayer) currentDesignItem = DesignItem.GetOrCreateDesignItem(currentElement, this.serviceContainer, this.instanceServiceContainer);
    } else {
      this._resetPointerEventsForClickThrough();
    }
