...
export class DesignerView extends BaseCustomWebComponentLazyAppend {
  ...
  async executeCommand(command) {
    switch (command.type) {
      case CommandType.screenshot:
        {
          if (!this.instanceServiceContainer.selectionService.primarySelection) alert("you need to select an element!");else {
  ...
  _addDesignItems(designItems) {
    ...
    if (prepService) requestAnimationFrame(() => prepService.prepareElementsForDesigner(this.rootDesignItem));
  }
  ...
  _onDragOver(event) {
    ...
    if (event.dataTransfer.types.length > 0 && event.dataTransfer.types[0] == 'Files') {
      ...
      if (ddService) {
        ...
        if (effect !== 'none') this._canvas.classList.add('dragFileActive');
      }
    }
  }
  ...
  onKeyUp(event) {
    if (event.composedPath().indexOf(this.eatEvents) >= 0) return;
  ...
  onKeyDown(event) {
    if (event.composedPath().indexOf(this.eatEvents) >= 0) return; //TODO: keyboard events maybe should also be handeled by tools 
        ...
        if (currentElement.shadowRoot) {
          for (let e of currentElement.shadowRoot.querySelectorAll('*')) {
            if (!backupPEventsMap.has(e)) {
              if (e.style) backupPEventsMap.set(e, e.style.pointerEvents);
  ...
  _pointerEventHandler(event) {
    ...
    let currentElement = this.serviceContainer.elementAtPointService.getElementAtPoint(this, {
      x: event.x,
      y: event.y
    });
    ...
    if (this._lastHoverDesignItem != currentDesignItem) {
      ...
      if (currentDesignItem && currentDesignItem != this.rootDesignItem && DesignerView.getHost(currentElement.parentNode) !== this.overlayLayer) this.extensionManager.applyExtension(currentDesignItem, ExtensionType.MouseOver);
      this._lastHoverDesignItem = currentDesignItem;
    }

    if (currentElement && DesignerView.getHost(currentElement.parentNode) === this.overlayLayer) currentElement = (_a = this.instanceServiceContainer.selectionService.primarySelection.element) !== null && _a !== void 0 ? _a : this._canvas;
